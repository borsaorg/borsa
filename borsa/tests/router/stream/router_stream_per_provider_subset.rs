use crate::helpers::{instrument, AAPL, MSFT, usd};
...
async fn stream_quotes_assigns_symbols_per_provider() {
    ...
    let (_h, mut rx) = borsa
        .stream_quotes(&[instrument(AAPL, AssetKind::Equity), instrument(MSFT, AssetKind::Equity)])
        .await
    ...
    while let Some(u) = rx.recv().await {
        got.push((u.symbol.as_str().to_string(), u.price.as_ref().map(|m| m.amount().to_string())));
        if got.len() >= 2 { break; }
    }
